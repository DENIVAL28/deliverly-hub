-- Números de pedido sequenciais por empresa (cada loja começa do 1)

-- 1. Adiciona contador por empresa (DEFAULT 0 — incrementado antes de usar)
ALTER TABLE public.empresas ADD COLUMN IF NOT EXISTS proximo_numero INT NOT NULL DEFAULT 0;

-- 2. Inicializa empresas existentes com o maior numero atual de seus pedidos
UPDATE public.empresas e
SET proximo_numero = COALESCE((
  SELECT MAX(p.numero) FROM public.pedidos p WHERE p.empresa_id = e.id
), 0);

-- 3. Remove o default SERIAL de pedidos.numero (passa a ser definido manualmente)
ALTER TABLE public.pedidos ALTER COLUMN numero DROP DEFAULT;

-- 4. Recria finalizar_pedido v5 — usa contador por empresa
...
BEGIN
  ...
  IF NOT FOUND THEN
    -- Empresa não existe ou não está ativa
  ...
  -- ── 6. Insere pedido com número sequencial por empresa ────────────────────────
