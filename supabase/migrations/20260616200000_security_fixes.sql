-- ═══════════════════════════════════════════════════════════════════════════════
-- Security fixes v1
-- 1. finalizar_pedido — exige auth.uid() para desconto PDV (anon não desconta)
-- 2. analytics_eventos — INSERT restrito a empresas ativas
-- 3. entregador_atualizar_pix — RPC seguro em vez de UPDATE direto
-- ═══════════════════════════════════════════════════════════════════════════════

SET search_path = public;

-- ── 1. finalizar_pedido v3.1 ──────────────────────────────────────────────────
...
BEGIN
  ...
  ELSIF p_tipo = 'pdv' AND p_desconto_pdv > 0 THEN
    -- FIX: desconto PDV exige usuário autenticado — anon não pode zerar o preço
...
-- ── 3. entregador_atualizar_pix — RPC seguro por token ────────────────────────

CREATE OR REPLACE FUNCTION public.entregador_atualizar_pix(
  p_token          uuid,
  p_chave_pix      text,
  p_tipo_chave_pix text DEFAULT 'aleatoria'
) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_id uuid;
BEGIN
  IF p_tipo_chave_pix NOT IN ('aleatoria', 'cpf', 'telefone', 'email') THEN
    RAISE EXCEPTION 'Tipo de chave PIX inválido.';
  END IF;
  IF char_length(COALESCE(p_chave_pix, '')) > 150 THEN
  ...
  UPDATE public.entregadores
     SET chave_pix = p_chave_pix, tipo_chave_pix = p_tipo_chave_pix
   WHERE id = v_id;
END;
$$;

GRANT EXECUTE ON FUNCTION public.entregador_atualizar_pix(uuid, text, text) TO anon, authenticated;
