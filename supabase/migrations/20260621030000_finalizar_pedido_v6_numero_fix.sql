-- finalizar_pedido v6: combina numero-por-empresa (v5 de 20260617) + p_taxa_entrega PDV.
-- A v5 de 20260621 foi baseada na v4 antiga que usava sequence — numero ficava NULL.
...
BEGIN
  ...
  ELSIF p_tipo = 'pdv' THEN
    -- PDV: taxa manual informada pelo lojista (entrega domiciliar por telefone)
  ...
  IF p_cupom_id IS NOT NULL THEN
    ...
    IF v_cupom.usos_max IS NOT NULL AND COALESCE(v_cupom.usos_atual, 0) >= v_cupom.usos_atual THEN RAISE EXCEPTION 'Este cupom atingiu o limite de usos.'; END IF;
