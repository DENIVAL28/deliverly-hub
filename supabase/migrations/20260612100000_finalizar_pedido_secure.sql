-- ═══════════════════════════════════════════════════════════════════════════════
-- finalizar_pedido v3 — recalculação server-side, validações de segurança
-- Não confiar em p_subtotal / p_taxa_entrega / p_total / preco_unitario do client
-- ═══════════════════════════════════════════════════════════════════════════════

SET search_path = public;

-- Adiciona colunas novas em pedidos (idempotente)
...
-- Remove todas as assinaturas anteriores da função
DROP FUNCTION IF EXISTS public.finalizar_pedido(uuid,text,text,text,text,text,numeric,numeric,numeric,text,text,text,uuid,jsonb);
DROP FUNCTION IF EXISTS public.finalizar_pedido(uuid,text,text,text,text,text,numeric,numeric,numeric,text,text,text,uuid,jsonb,double precision,double precision);
DROP FUNCTION IF EXISTS public.finalizar_pedido(uuid,text,text,text,text,text,numeric,numeric,numeric,text,text,text,uuid,jsonb,double precision,double precision,text,text,text);
...
BEGIN
  ...
  -- ── 1. Valida empresa ─────────────────────────────────────────────────────────
  SELECT status, taxa_entrega
    INTO v_emp_status, v_emp_taxa
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_itens) LOOP
    ...
    -- Bloqueia a linha para evitar race condition no estoque
    SELECT nome,
  ...
  -- ── 3. Taxa de entrega do banco (nunca do client) ─────────────────────────────
  ...
  ELSIF p_tipo = 'pdv' AND p_desconto_pdv > 0 THEN
    -- Desconto manual do operador PDV (usuário autenticado)
  ...
  -- ── 6. Insere pedido (status sempre 'novo') ────────────────────────────────────
