-- Taxa de entrega por distância (km) ou fixa
ALTER TABLE public.empresas
  ADD COLUMN IF NOT EXISTS taxa_entrega_tipo   TEXT    NOT NULL DEFAULT 'fixo' CHECK (taxa_entrega_tipo IN ('fixo', 'km')),
  ADD COLUMN IF NOT EXISTS taxa_entrega_por_km NUMERIC(10,2) NOT NULL DEFAULT 2,
  ADD COLUMN IF NOT EXISTS taxa_entrega_base   NUMERIC(10,2) NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS empresa_lat         DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS empresa_lng         DOUBLE PRECISION;
...
-- Atualiza finalizar_pedido para suportar taxa por km
DROP FUNCTION IF EXISTS public.finalizar_pedido(uuid,text,text,text,text,text,text,text,uuid,jsonb,double precision,double precision,text,text,text,numeric);
...
BEGIN
  ...
  -- ── 1. Valida empresa ─────────────────────────────────────────────────────────
  SELECT status, taxa_entrega, taxa_entrega_tipo, taxa_entrega_por_km, taxa_entrega_base, empresa_lat, empresa_lng
    INTO v_emp_status, v_emp_taxa, v_taxa_tipo, v_taxa_por_km, v_taxa_base, v_emp_lat, v_emp_lng
