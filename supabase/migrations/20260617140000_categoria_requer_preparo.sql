-- Categorias: flag requer_preparo (default true)
-- Bebidas, itens prontos etc. podem ter requer_preparo = false
ALTER TABLE public.categorias
  ADD COLUMN IF NOT EXISTS requer_preparo BOOLEAN NOT NULL DEFAULT true;

-- Produtos sem categoria herdam requer_preparo = true (já é o default de pedido_itens)

-- Recria finalizar_pedido v7 — requer_preparo vem da categoria do produto
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_itens) LOOP
    ...
    SELECT p.nome, COALESCE(p.preco_promocional, p.preco), p.ativo, p.controlar_estoque, p.estoque, p.categoria_id
      INTO v_prod_nome, v_preco, v_ativo, v_ctrl_est, v_estoque, v_cat_id
      FROM public.produtos p
     WHERE p.id = v_prod_id AND p.empresa_id = p_empresa_id FOR UPDATE;
    ...
    -- requer_preparo vem da categoria; sem categoria = true
    SELECT COALESCE(c.requer_preparo, true)
      INTO v_req_preparo
      FROM public.categorias c
     WHERE c.id = v_cat_id;
    IF NOT FOUND THEN v_req_preparo := true; END IF;
  ...
  -- Insere itens com requer_preparo da categoria
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_itens) LOOP
    v_prod_id := (v_item->>'produto_id')::UUID;
    v_qty     := (v_item->>'quantidade')::INT;

    SELECT p.nome, COALESCE(p.preco_promocional, p.preco), p.categoria_id
      INTO v_prod_nome, v_preco, v_cat_id
      FROM public.produtos p WHERE p.id = v_prod_id AND p.empresa_id = p_empresa_id;

    SELECT COALESCE(c.requer_preparo, true) INTO v_req_preparo
      FROM public.categorias c WHERE c.id = v_cat_id;
    IF NOT FOUND THEN v_req_preparo := true; END IF;
    ...
    INSERT INTO public.pedido_itens (
      ...
      NULLIF(v_item->>'observacao', ''), v_req_preparo
    );
  END LOOP;
